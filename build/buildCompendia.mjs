...
const dirPaths = fs.readdirSync(dataPath).map((name) => path.resolve(dirName, dataPath, name));
...
console.log(
	`[INFO] - Successfully built ${counts.length} packs with a total of ${totalCount} documents.`,
);
