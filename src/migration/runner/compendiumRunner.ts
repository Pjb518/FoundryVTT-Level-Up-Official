...
class MigrationRunnerCompendium extends MigrationRunnerBase {
  ...
  #isItem(doc: Record<string, any>): boolean {
    return (
      "system" in doc && doc.system && !this.#isActor(doc) && !("text" in doc)
    );
  }
  ...
  async getUpdatedSource(
    ...
    if (this.#isActor(source)) {
      ...
      return foundry.utils.mergeObject(source, update, {
        inplace: false,
  ...
  async runMigration(dirPath: string) {
    ...
    for (const [filePath, source] of files.entries()) {
      ...
      // Write to file
      if (!foundry.utils.equals(source, updated)) {
        fs.writeFileSync(filePath, this.#toFixedJSON(updated), {
          encoding: "utf-8",
        });
      }
    }
