...
export default class MigrationRunner extends MigrationRunnerBase {
	...
	async #migrateActor(migrations, actor, options = {}) {
		...
		if (itemDiff.deleted.length > 0) {
			try {
				const finalDeleted = itemDiff.deleted.filter((deleteId) =>
					actor.items.some((item) => item.id === deleteId),
				);
		...
		if (effectDiff.deleted.length > 0) {
			try {
				const finalDeleted = effectDiff.deleted.filter((deleteId) =>
					actor.effects.some((effect) => effect.id === deleteId),
				);
		...
		if (effectDiff.inserted.length > 0) {
			try {
				await actor.createEmbeddedDocuments('ActiveEffect', effectDiff.inserted, {
					noHook: true,
					pack,
				});
			} catch (e) {
	...
	async #migrateItem(migrations, item, options = {}) {
		...
		if (effectDiff.deleted.length > 0) {
			try {
				const finalDeleted = effectDiff.deleted.filter((deleteId) =>
					item.effects.some((effect) => effect.id === deleteId),
				);
		...
		if (effectDiff.inserted.length > 0) {
			try {
				await item.createEmbeddedDocuments('ActiveEffect', effectDiff.inserted, {
					noHook: true,
					pack,
				});
			} catch (e) {
	...
	async runCompendiumMigration(compendium, legacyMigrate = false) {
		...
		if (legacyMigrate) {
			...
			lowestSchemaVersion = Math.min(
				MigrationRunnerBase.LATEST_SCHEMA_VERSION,
				...documents.map((d) => d.system?.schema?.version ?? 0.0).filter((d) => !!d),
			);
		}

		console.log(`A5E | Lowest Schema Version: ${lowestSchemaVersion}`);
		const migrations = this.migrations.filter(
			(migration) => migration.version > lowestSchemaVersion,
		);
