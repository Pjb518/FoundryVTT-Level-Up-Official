...
export class Migration010MigrateContexts extends MigrationBase {
	...
	#updateSkillBonuses(actorData) {
		...
		const { passives, checks } = Object.entries(skills).reduce(
			(acc, [skillId, skill]) => {
	...
	#getEffectAbilityChange(change) {
		...
		const isIndividualCheck =
			change.key.startsWith('system.abilities.') && change.key.endsWith('.check');
		const isIndividualSave =
			change.key.startsWith('system.abilities.') && change.key.endsWith('.save');
	...
	#getEffectSkillChange(change) {
		...
		const isIndividualPassive =
			change.key.startsWith('system.skills.') && change.key.endsWith('.passive');
		const formula = change.value;
	...
	async updateEffect(effectData) {
		...
		changes.forEach((change, idx) => {
			...
			if (key === 'flags.a5e.effects.bonuses.damage') {
				...
				isAbilityBonus ||
				(parts.length !== 5 && ['check', 'save'].includes(parts.at(-1)))
			) {
