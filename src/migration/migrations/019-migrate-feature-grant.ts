...
export class Migration019MigrateFeatureGrant extends MigrationBase {
	...
	async updateItem(item: Record<string, any>) {
		...
		Object.entries(grants).forEach(([grantId, grant]) => {
			...
			const { base = [], options = [] }: { base: string[]; options: string[] } =
				grant.features ?? {};

			const baseFeatures = base.map((uuid) => ({
				uuid,
				limitedReselection: true,
				selectionLimit: 1,
			}));

			const optionalFeatures = options.map((uuid) => ({
				uuid,
				limitedReselection: true,
				selectionLimit: 1,
			}));
