...
export default function modifyDerivedOptions(options: Object) {
  ...
  options["system.attributes.ac.changes.bonuses.value"] = [
    0,
    MODES.DEFAULT_MODES,
  ];
  ...
  Object.keys(CONFIG.A5E.movement).forEach((m) => {
    options[`system.attributes.movement.${m}.distance`] = [
      0,
      MODES.DEFAULT_MODES,
    ];
    ...
    options["flags.a5e.effects.movement.allDistances"] = [
      0,
      MODES.DEFAULT_MODES,
    ];
  ...
  Object.keys(CONFIG.A5E.senses).forEach((s) => {
    options[`system.attributes.senses.${s}.distance`] = [
      0,
      MODES.DEFAULT_MODES,
    ];
