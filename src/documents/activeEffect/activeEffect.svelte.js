...
export default class ActiveEffectA5e extends ActiveEffect {
  ...
  get isSuppressed() {
    if (
      this.disabled ||
      !["Actor", "ActorDelta", "Token"].includes(this.parent.documentName)
    )
      return true;
  ...
  apply(document, _change, phase = "applyAEs") {
    ...
    if (phase === "applyAEs") {
      const val = foundry.utils
        .deepClone(change.value)
        .replace("@original", "");

      const resValue = Roll.replaceFormulaData(val, document.getRollData(), {
        missing: null,
      });
  ...
  #applyConditional(document, current, change) {
    ...
    const compareValue = getDeterministicBonus(
      obj.comparisonValue ?? "0",
      document.getRollData(),
    );
    const operator = obj.comparisonOperator ?? "==";
    positiveValue = getDeterministicBonus(
      positiveValue ?? "0",
      document.getRollData(),
    );
    negativeValue = getDeterministicBonus(
      negativeValue ?? "0",
      document.getRollData(),
    );
  ...
  #convertToDeterministicBonus(document, change) {
    ...
    try {
      if (isActor) {
        const targetField =
          game.a5e.activeEffects.options[document.type].allOptions[change.key];
        if (typeof targetField.sampleValue !== "number") return change.value;

        return (
          getDeterministicBonus(change.value ?? 0, document.getRollData()) ??
          change.value
        );
      }
    ...
    applyObjects.forEach(({ change, effect }) => {
      ...
      if (change.key === "system.attributes.spellDC") {
        const spellBookIds = Object.keys(
          effect.parent?.system?.spellBooks ?? {},
        );
  ...
  static applyEffects(
    ...
    applyObjects.sort(
      (a, b) => (a.change.priority ?? 0) - (b.change.priority ?? 0),
    );
    ...
    applyObjects.forEach((applyObject) => {
      ...
      if (
        document.documentName === "Token" &&
        currentPhase === "afterDerived"
      ) {
        appliedChange = applyObject.effect.apply(
          document,
          applyObject.change,
          currentPhase,
        );
        ...
        if (appliedChange === null && !applyObject.effect.isSuppressed) {
          if (!nextPhase) {
            ui.notifications.error(
              localize("A5E.notifications.effects.invalidChange"),
            );
            return;
          }
          ...
          let idx =
            document.effectPhases[nextPhase]?.findIndex(
              (e) =>
                e.effect._id === applyObject.effect._id &&
