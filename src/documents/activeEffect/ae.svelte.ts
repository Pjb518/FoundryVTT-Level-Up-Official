...
class ActiveEffectA5E extends ActiveEffect {
	...
	constructor(data: any, context = {}) {
		...
		this.#subscribe = createSubscriber((update) => {
			const updateEffectHook = Hooks.on('updateActiveEffect', (triggeringDocument, _, { diff }) => {
	...
	static CHANGE_TYPES = {
		...
		conditional: {
			...
			handler: (
				...
				const current = field.value ?? foundry.utils.getProperty(targetDoc, change.key) ?? 0;

				let trueValue = getDeterministicBonus(operationData.positiveValue, replacementData);
				let falseValue = getDeterministicBonus(operationData.negativeValue, replacementData);
				...
				const result = evaluateConditional(current, operator, compareValue, trueValue, falseValue);
	...
	get active() {
		...
		if (item.system.equippedState !== CONFIG.A5E.EQUIPPED_STATES.EQUIPPED) return false;
	...
	static applyChange(targetDoc, change, { replacementData = {}, modifyTarget = true }) {
		...
		const configuredHandler = ActiveEffectA5E.CHANGE_TYPES[change.type]?.handler;
	...
	static #castDelta(raw: any, type: string, replacementData = {}) {
		let delta;
		if (typeof raw === 'string') raw = super._replaceDataRefs(raw, replacementData);
