...
class BaseActorA5e extends Actor {
  ...
  override get temporaryEffects() {
    ...
    for (const effect of this.allApplicableEffects()) {
      if (
        effect.active &&
        (effect.isTemporary || effect?.system?.effectType === "onUse")
      ) {
        effects.push(effect);
      }
    }
    ...
    Object.entries(actorData.abilities).forEach(([abilityKey, ability]) => {
      const value = getDeterministicBonus(
        [
          // @ts-expect-error
          ability.value,
          this.BonusesManager?.getAbilityBonusesFormula(
            abilityKey,
            "base",
          ).trim(),
        ]
    ...
    try {
      ...
      actorData.attributes.spellDC = getDeterministicBonus(
        [
          ...
          actorData.abilities[actorData.attributes.spellcasting || "int"].check
            .mod,
        ].join(" + "),
        this.getRollData(),
      );
  ...
  prepareArmorClass() {
    ...
    const valueOverride = foundry.utils.getProperty(
      this.overrides,
    ...
    if (valueOverride !== null && valueOverride !== undefined) {
      const effectOverride = this.actorEffects.findLast(
        (effect) =>
          effect.changes.some((change) => change.key.includes("ac.value")) &&
          !effect.isSuppressed,
      );

      const tempFinalAC =
        (changes.override?.value ?? baseAC) + changes.bonuses.value;
      ...
      const effectOverride = this.actorEffects.findLast(
        (effect) =>
          effect.changes.some((change) =>
            change.key.includes("ac.baseFormula"),
          ) && !effect.isSuppressed,
      );
    ...
    const changes = this.items.reduce(
      (acc, item) => {
        ...
        if (
          ...
          return acc;
        if ((requiresUnarmored && hasArmor) || (requiresNoShield && hasShield))
          return acc;
        ...
        if (item.system.objectType === "armor") {
          // @ts-expect-error
          const isUnderArmor =
            item.system.materialProperties.includes("underarmor");
        ...
        // @ts-expect-error
          ...
          item.system.objectType === "shield" &&
          value > (acc.shield?.value ?? 0)
        ) {
          // @ts-expect-error
          acc.shield = change;
          // @ts-expect-error
  ...
  prepareSkills() {
    ...
    Object.entries(actorData.skills).forEach(([key, skill]) => {
      ...
      try {
        deterministicBonus = getDeterministicBonus(
          [
            ...
            skill.mod,
            // @ts-expect-error
            this.BonusesManager?.getSkillBonusesFormula(
              key,
              skill.ability,
              "check",
              true,
        ...
        // eslint-disable-next-line no-console
        console.error(
          `Couldn't calculate a ${skillName} modifier for ${this.name}`,
        );
      }
      ...
      try {
        ...
        // eslint-disable-next-line no-console
        console.error(
          `Couldn't calculate a ${skillName} passive score for ${this.name}`,
        );
        // @ts-expect-error
        skill.passive = null;
      }
  ...
  prepareMovement() {
    ...
    deferApplication.forEach((type) => {
      ...
      const bonus = getDeterministicBonus(bonusFormula, this.getRollData());
      if (!bonus) return;
      this.system.attributes.movement[type].distance =
        movement[type].distance + bonus;
    });
  }
  ...
  prepareSenses() {
    ...
    deferApplication.forEach((type) => {
      ...
      const bonus = getDeterministicBonus(bonusFormula, this.getRollData());
      if (!bonus) return;
      this.system.attributes.senses[type].distance =
        senses[type].distance + bonus;
    });
  }
  ...
  async applyBulkDamage(damageRolls) {
    ...
    if (game.settings.get("a5e", "enableCascadingDamageAndHealing")) {
      const delayDelta = game.settings.get(
        "a5e",
  ...
  async triggerRest(restOptions: ActorRestOptions = {}) {
    ...
    if (foundry.utils.isEmpty(restOptions)) {
      const title = localize("A5E.RestConfigurationPrompt", {
        name: this.name,
      });
  ...
  async rechargeGenericResource(resource: string) {
    ...
    const max =
      getDeterministicBonus(
        this.system.resources[resource].max,
        this.getRollData(),
      ) ?? 0;
    ...
    const rechargeType =
      this.system.resources[resource]?.recharge?.rechargeType || "custom";
    const rechargeAmount =
      this.system.resources[resource]?.recharge?.rechargeAmount || "1";
  ...
  async rollAbilityCheck(
    ...
    if (options.skipRollDialog)
      dialogData = this.getDefaultAbilityCheckData(abilityKey, options);
  ...
  async rollSavingThrow(
    ...
    if (options.skipRollDialog)
      dialogData = this.getDefaultSavingThrowData(abilityKey, options);
    else dialogData = await this.#showSavingThrowPrompt(abilityKey, options);

    if (dialogData === null) return null;

    const { expertiseDie, rollFormula, rollMode, saveType, visibilityMode } =
      dialogData;
  ...
  getDefaultSavingThrowData(
    ...
    const rollOverrideKey = abilityKey
      ? `system.abilities.${abilityKey}.save`
      : "deathSave";
    const rollMode = this.RollOverrideManager?.getRollOverride(
      rollOverrideKey,
      defaultRollMode,
  ...
  async rollSkillCheck(skillKey: string, options: SkillCheckRollOptions = {}) {
    ...
    if (options.skipRollDialog)
      dialogData = this.getDefaultSkillCheckData(skillKey, options);
    else dialogData = await this.#showSkillCheckPrompt(skillKey, options);

    if (!dialogData) return null;

    const { abilityKey, expertiseDie, rollFormula, rollMode, visibilityMode } =
      dialogData;
  ...
  getDefaultSkillCheckData(
    ...
    const defaultRollMode = options?.rollMode ?? CONFIG.A5E.ROLL_MODE.NORMAL;
    const defaultExpertiseDie =
      options?.expertiseDice ?? skill.expertiseDice ?? 0;
  ...
  addBonus(type = "damage") {
    const bonuses = foundry.utils.duplicate(
      this._source.system.bonuses[type] ?? {},
    );
  ...
  configureAlignment(data: Record<string, any> = {}, options = {}) {
    const title = localize("A5E.AlignmentConfigurationPrompt", {
      name: this.name,
    });
  ...
  configureArmorProficiencies(data: Record<string, any> = {}, options = {}) {
    const title = localize("A5E.ArmorProficienciesConfigurationPrompt", {
      name: this.name,
    });
  ...
  configureConditionImmunities(data: Record<string, any> = {}, options = {}) {
    const title = localize("A5E.ConditionImmunitiesConfigurationPrompt", {
      name: this.name,
    });
  ...
  configureCreatureTypes(data: Record<string, any> = {}, options = {}) {
    const title = localize("A5E.CreatureTypesConfigurationPrompt", {
      name: this.name,
    });
  ...
  configureCreatureTerrains(data: Record<string, any> = {}, options = {}) {
    ...
    this.#configure(
      "terrain",
      `${this.name}: Configure Creature Terrains`,
      data,
      options,
    );
  }
  ...
  configureDamageImmunities(data: Record<string, any> = {}, options = {}) {
    const title = localize("A5E.DamageImmunitiesConfigurationPrompt", {
      name: this.name,
    });
  ...
  configureDamageResistances(data: Record<string, any> = {}, options = {}) {
    const title = localize("A5E.DamageResistancesConfigurationPrompt", {
      name: this.name,
    });
  ...
  configureDamageVulnerabilities(data: Record<string, any> = {}, options = {}) {
    const title = localize("A5E.DamageVulnerabilitiesConfigurationPrompt", {
      name: this.name,
    });
  ...
  configureHealth(
    ...
    const title = localize("A5E.HitPointsConfigurationPrompt", {
      name: this.name,
    });
    options.width ??= 380;
  ...
  configureInitiative(
    ...
    const title = localize("A5E.InitiativeConfigurationPrompt", {
      name: this.name,
    });
  ...
  configureLanguages(data: Record<string, any> = {}, options = {}) {
    const title = localize("A5E.LanguagesConfigurationPrompt", {
      name: this.name,
    });
  ...
  configureMovement(data: Record<string, any> = {}, options = {}) {
    const title = localize("A5E.MovementConfigurationPrompt", {
      name: this.name,
    });
  ...
  configureManeuverTraditions(data: Record<string, any> = {}, options = {}) {
    const title = localize("A5E.ManeuverTraditionsConfigurationPrompt", {
      name: this.name,
    });
  ...
  configureSenses(data: Record<string, any> = {}, options = {}) {
    const title = localize("A5E.SensesConfigurationPrompt", {
      name: this.name,
    });
  ...
  configureSizeCategory(data: Record<string, any> = {}, options = {}) {
    const title = localize("A5E.SizeCategoryConfigurationPrompt", {
      name: this.name,
    });
  ...
  async toggleStatusEffect(
    ...
    if (!status)
      throw new Error(
        `Invalid status ID "${statusId}" provided to Actor#toggleStatusEffect`,
      );
    ...
    if (
      ["corruption", "fatigue", "exhaustion", "inebriated", "strife"].includes(
        statusId,
      )
    ) {
