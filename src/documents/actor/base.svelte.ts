...
import type CharacterActorA5E from './character.ts';
import type {
...
import type NPCActorA5E from './npc.ts';
...
type ActorMap = {
	...
	npc: NPCActorA5E;
  base: BaseActorA5e;
	[K: foundry.abstract.Document.ModuleSubType]: BaseItemA5e<foundry.abstract.Document.ModuleSubType>;
};
...
declare interface ActorA5e extends Identity<BaseActorA5e> {
 ...
 */
override;
applyActiveEffects(phase: 'initial' | 'final')
{
 ...
 */
async;
applyDamage(damage, (damageType = null));
{
	const updates = {};
	const { value, temp } = this.system.attributes.hp;
 ...
 */
async;
applyHealing(healing, healingType?: string)
{
	const updates = {};
	// @ts-expect-error
	const { value, max, temp } = this.system.attributes.hp;
...
/** @inheritdoc */
override;
getRollData(item: BaseItemA5e | null = null)
{
...
override;
async;
modifyTokenAttribute(attribute: string, value, isDelta: boolean, isBar: boolean)
{
...
// -------------------------------------------------------------
async;
triggerRest(restOptions: ActorRestOptions = {})
{
...
async;
updateDeathSavingThrowFigures(roll);
{
...
async;
rechargeGenericResource(resource: string)
{
 ...
 */
async;
rollAbilityCheck(abilityKey: string, options: AbilityCheckRollOptions = {})
{
	let dialogData;
...
async;
rollDeathSavingThrow(options: SavingThrowRollOptions = {})
{
...
async;
rollHitDice(dieSize: string | null = null, quantity = 1, heal = true)
{
...
async;
rollSavingThrow(abilityKey?: string, options: SavingThrowRollOptions = {})
{
	let dialogData;
 ...
 */
async;
rollSkillCheck(skillKey: string, options: SkillCheckRollOptions = {})
{
	let dialogData;
...
async;
deleteBonus(id: string, type = 'damage')
{
	// Close dialog
