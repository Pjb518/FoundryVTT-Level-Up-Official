...
export default class ObjectItemA5e extends ItemA5e {
  ...
  get weight() {
    if (this.system.objectType === "container") {
      ...
      if (w instanceof Promise)
        return w.then((cw) => (cw ?? 0) + this.system.weight);
  ...
  get container() {
    ...
    if (this.pack)
      return game.packs.get(this.pack)?.getDocument(this.system.containerId);
  ...
  get containerItemNames() {
    ...
    const names = ((this.containerItems.allItems as any[]) ?? []).map(
      (i) => i.name,
    );
  ...
  override async duplicateItem() {
    ...
    const container = await ContainerManager.createContainerOnActor(
      this.parent,
      this,
    );
    return container;
  ...
  async toggleEquippedState() {
    ...
    if (
      newState === CONFIG.A5E.EQUIPPED_STATES.EQUIPPED &&
      this.system.objectType === "armor"
    ) {
      ...
      const isUnderarmor =
        this.system.materialProperties.includes("underarmor");
      ...
      if (newState === 0) {
        ...
        ui.notifications.warn(
          game.i18n.localize("A5E.armorClass.armorAlreadyEquipped"),
        );
      }
    }
    ...
    if (newState === 2 && this.system.objectType === "shield") {
      ...
      if (newState === 0) {
        // @ts-expect-error
        ui.notifications.warn(
          game.i18n.localize("A5E.armorClass.shieldAlreadyEquipped"),
        );
      }
    }
  ...
  async updateContainer(containerUuid: string) {
    ...
    const container = (await fromUuid(containerUuid)) as InstanceType<
  ...
  override async _preUpdate(data, options, user) {
    if (foundry.utils.getProperty(data, "system.objectType"))
      await this._preUpdateObjectType();
  ...
  override async _onCreate(data, options, userId) {
    ...
    actions.forEach(([actionId, action]) => {
      ...
      consumers.forEach(([consumerId, consumer]) => {
        ...
        updates[`system.actions.${actionId}.consumers.${consumerId}.itemId`] =
          this._id;
      });
    });
