import { A5EArchetypeData } from '../../dataModels/item/ArchetypeDataModel.ts';
import type { A5EClassData } from '../../dataModels/item/ClassDataModel.ts';
import { getDeterministicBonus } from '../../dice/getDeterministicBonus.ts';
import ClassResourceManager from '../../managers/ClassResourceManager.ts';
import type ArchetypeItemA5e from './archetype.ts';
import type { ClassCastingData } from './data.ts';
import OriginItemA5e from './origin.ts';
...
export default class ClassItemA5e extends OriginItemA5e<'class'> {
	...
	override async _preCreate(data, options, user): Promise<boolean | void> {
		...
		if (this.parent?.documentName === 'Actor') {
			...
			if (existing) {
				existing.update({
					'system.classLevels': Math.min(existing.system.classLevels + 1, 20),
				});
				return false;
			}
		}
	...
	override async _preUpdate(changed, options, user): Promise<boolean | void> {
		...
		if (keys.includes('system.hp.hitDiceUsed')) {
			...
			changed.system.hp.hitDiceUsed = Math.clamp(used, 0, max);
		}
