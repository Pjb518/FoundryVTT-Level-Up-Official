...
export default class ClassItemA5e extends OriginItemA5e {
  ...
  override async _preUpdate(changed, options, user): Promise<boolean | void> {
    ...
    if (
      keys.includes("system.hp.hitDiceSize") &&
      (this.isStartingClass || !this.parent)
    ) {
