...
class ItemA5e extends BaseItemA5e {
  ...
  override async activate(
    ...
    if (
      ...
      // @ts-expect-error
      options.executeMacro ??=
        this.system.macro.trim().length > 0 || this.actions.hasMacro;
    }
  ...
  async #activateAction(
    ...
    if (options.skipRollDialog) {
      ...
      activationData = await this.#showActionActivationPrompt(
        actionId,
        options,
      );
    }
  ...
  async #showActionActivationPrompt(
    actionId: string,
    options: ActionActivationOptions,
  ) {
  ...
  #getDefaultActionActivationData(
    actionId: string,
    options: ActionActivationOptions,
  ) {
    const action = this.actions.get(actionId);
    ...
    const { damageBonuses, healingBonuses } = this.#getDefaultBonuses(
      this.actor,
      rolls,
    );
  ...
  #getDefaultConsumerData(actionId: string) {
    ...
    const { actionUsesData, itemUsesData } =
    ...
    const { hitDiceData } = ResourceConsumptionManager.prepareHitDiceData(
      this.actor!,
      consumers,
    );
    ...
    const {
      spellData,
      // @ts-expect-error
    } = ResourceConsumptionManager.prepareSpellData(
      this.actor!,
      this,
      consumers,
      actionId,
  ...
  #getDefaultPrompts(actionId: string) {
    ...
    return Object.entries(promptsByType).reduce(
      (defaultPrompts, [promptType, promptGroup]) => {
        defaultPrompts.push(
  ...
  #getDefaultRollData(rolls: RollHandlerReturnType) {
    return Object.entries(rolls).reduce(
      (defaultRolls, [rollType, rollGroup]) => {
  ...
  async recharge(actionId: string, state = false) {
    ...
    // Recharge Roll
    const rechargeRoll = await new Roll(
      formula,
      this.actor.getRollData(this),
    ).evaluate();
