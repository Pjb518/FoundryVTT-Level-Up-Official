...
export default class ActorSheet extends SvelteApplicationMixin(
	...
	constructor(actor: { document: any }, options: any = {}) {
		...
		if (
			[CONST.DOCUMENT_OWNERSHIP_LEVELS.NONE, CONST.DOCUMENT_OWNERSHIP_LEVELS.LIMITED].includes(
				actor.document.permission,
			)
		) {
			options.classes = ['a5e-sheet', 'a5e-sheet--actor', 'a5e-sheet--actor-limited'];
	...
	_sortEmbeddedAlphabetically(docs: any[], documentName: 'Item' | 'ActiveEffect', reverse = false) {
		const entities = [...docs];
	...
	override async _onDropItem(event: DragEvent, item: Item) {
		...
		if (this.actor.uuid === item.parent?.uuid && !options.containerUuid && !options.isMovingOut) {
	...
	async #getDragOptions(event: DragEvent, item?: Item) {
		...
		const currentSpellBook: string =
			this.tempSettings?.currentSpellBook ?? Object.keys(this.actor?.system?.spellBooks)?.[0];
		...
		const containerId = isContainer ? targetUUID : targetItem?.system?.containerId;
