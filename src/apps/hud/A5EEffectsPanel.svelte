...
function removeEffect(id) {
	...
	if (statuses.size === 1) {
		if (subConditions[statuses.first()]?.some((c) => activeConditions.includes(c))) return;
...
function getMaxEffectIconsPerColumn() {
	const fontSize = parseFloat(document.getElementsByTagName('html').style?.fontSize ?? 16);
