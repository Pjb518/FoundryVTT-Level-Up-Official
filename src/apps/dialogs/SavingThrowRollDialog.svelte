...
function getInitialExpertiseDieSelection() {
	...
	return $actor.RollOverrideManager.getExpertiseDice(
		rollModeKey ?? '',
		$actor.system.abilities[abilityKey]?.save?.expertiseDice || options.expertiseDice || 0,
	);
}
...
$: rollMode = $actor.RollOverrideManager.getRollOverride(rollModeKey, selectedRollMode);
