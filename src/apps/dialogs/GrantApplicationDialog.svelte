...
export let { allGrants, dialog, optionalGrantsProp, actor, item, cls, charLevel, clsLevel } =
	// @ts-ignore
...
function getApplicableGrants(activeGrants: Set<string>, selectedOptionalGrants: string[]) {
	const grantsList: {
		grant: Grant;
	...
	allGrants.forEach((grant: Grant) => {
		...
		const hasSelectionId = grantedBy?.selectionId ? uuids.includes(grantedBy.selectionId) : true;
...
function onSubmit() {
	const { updateData, documentData } = prepareGrantsApplyData(actor, grants, applyData) ?? {};
