...
async function toggleExpertiseDice(rollIndex, expertiseDice) {
	...
	const expertiseDie = expertiseDieIndex !== -1 ? newRoll.terms[expertiseDieIndex] : null;
...
const promptTypes = ['abilityCheck', 'savingThrow', 'skillCheck', 'generic', 'effect'];

const { actionDescription, itemDescription, unidentifiedDescription } = $message.flags?.a5e;
