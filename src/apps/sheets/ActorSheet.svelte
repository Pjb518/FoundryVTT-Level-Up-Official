...
function getTabs(actor: TJSDocument): Tab[] {
	return [
		...
		},
		{
			...
			hasSubNavigation: true,
			display:
				!actor.pack && actor.permission !== CONST.DOCUMENT_OWNERSHIP_LEVELS.OBSERVER.OBSERVER,
		},
	];
}
...
let currentTab = tabs.find((tab) => tab.name === tempSettings[$actor?.uuid]?.currentTab) ?? tabs[0];
