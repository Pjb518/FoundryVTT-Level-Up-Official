<script>
    ...
    const { poisonVectors } = CONFIG.A5E;
</script>

<Section
    heading="A5E.PoisonConfiguration"
    headerButtons={[
        {
...
>
    {#if editMode}
        <RadioGroup
            heading="A5E.PoisonVectors"
            options={Object.entries(poisonVectors)}
            selected={$item.system.poisonVectors}
            on:updateSelection={(event) =>
                updateDocumentDataFromField(
                    $item,
                    "system.poisonVectors",
                    event.detail,
                )}
        />
