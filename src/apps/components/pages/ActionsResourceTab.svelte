...
<section class="action-config action-config__wrapper">
  <header class="action-config__section-header">
    <h2 class="action-config__section-heading">{localize("A5E.Uses")}</h2>

    <button class="add-button" on:click={() => addResource()}>
      + Add Consumer
    </button>
  </header>

  <ul class="uses-list">
    {#each Object.entries(action.uses ?? {}) as [consumerId, consumer] (consumerId)}
