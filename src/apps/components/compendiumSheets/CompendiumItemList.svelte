...
const filterBy = {
	...
	magicItem: ['mundane', 'common', 'uncommon', 'rare', 'very rare', 'legendary', 'artifact'],
	maneuver: [0, 1, 2, 3, 4, 5],
	monster: [0, 0.125, 0.25, 0.5, ...Array.from(Array(30).keys(), (n) => n + 1)],
	object: ['mundane', 'common', 'uncommon', 'rare', 'very rare', 'legendary', 'artifact'],
...
function setupGrouping(enableGrouping) {
	...
	for (const filterValue of filters) {
		...
		dr.filters.add((doc) => foundry.utils.getProperty(doc, property) == filterValue);
