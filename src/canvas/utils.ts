...
export function squareAtPoint(point: Canvas.Point): PIXI.Rectangle {
	// @ts-expect-error
...
export function measureDistanceCuboid(
	...
	r1: PIXI.Rectangle,
	{
		...
		reach?: number | null;
		token?: TokenA5E | null;
		target?: TokenA5E | null;
	} = {},
): number {
	if (!canvas.grid) return -1;
	...
	if (rectanglesOverlap) {
		...
		distance.dx =
			Math.max(r0Snapped.left - r1Snapped.right, r1Snapped.left - r0Snapped.right, 0) + gridWidth;
		distance.dy =
			Math.max(r0Snapped.top - r1Snapped.bottom, r1Snapped.top - r0Snapped.bottom, 0) + gridWidth;
	}

	if (
		token &&
		target &&
		token.document.elevation !== target.document.elevation &&
		token.actor &&
		target.actor
	) {
		...
		const [selfDimensions, targetDimensions] = [token.dims, target.dims];
