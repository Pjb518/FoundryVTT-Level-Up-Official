...
export type SimpleMerge<Target, Override> = Omit<Target, keyof Override> &
  Override;

declare namespace RecordField {
  type Options<IValueField> = DataFieldOptions<
...
class RecordField<
  ...
  const PersistedType extends
    ...
    | null
    | undefined = RecordField.PersistedType<
    AssignmentElementType,
...
> {
  ...
  constructor(keyField: IKeyField, valueField: IValueField, options?: Options) {
    ...
    if (!(valueField instanceof foundry.data.fields.DataField))
      throw new Error(
        `${this.name} must have a DataField as its contained field`,
      );
  ...
  _isValidKeyFieldType(keyField) {
    if (
      ...
      if (
        keyField.options.required !== true ||
        keyField.options.nullable === true
      ) {
