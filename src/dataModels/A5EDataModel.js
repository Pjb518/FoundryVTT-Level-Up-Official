...
export default class A5EDataModel extends foundry.abstract.DataModel {
    ...
    _cleanType(data, options = {}) {
        ...
        for (const [name, field] of this.entries()) {
            if (!(name in data) && options.partial)
                continue;
    ...
    static mergeSchema(schema, template) {
        ...
        for (const key of Object.keys(template)) {
            ...
            if (fieldType === fields.SchemaField) {
                ...
                const elemOptions = foundry.utils.mergeObject(schema[key].element.options, template[key].element.options);
...
//# sourceMappingURL=A5EDataModel.js.map