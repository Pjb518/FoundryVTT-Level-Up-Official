<script lang="ts">
    ...
    import { preparePrompts } from "#utils/view/cards/cardPrompts/preparePrompts.ts";
    import { prepareRolls } from "#utils/view/cards/cardRolls/prepareRolls.ts";
    import { prepareSelectedTokenActors } from "#utils/view/helpers/prepareSelectedTokenActors.ts";
                    ...
                    <section class="prompt-button-wrapper">
                        {#each prompts[promptType] as prompt}
                            <PromptButton
                                ...
                                onTriggerPrompt={() => triggerPrompt(prompt)}
                            />
                        {/each}
                    </section>
                {/if}
    ...
    {#if hasEffects}
        {#each effects as effect}
            <PromptButton
                ...
                --hover-color={hoverColor}
                onTriggerPrompt={() => triggerEffect(effect)}
            />
        {/each}
    {/if}
</article>
