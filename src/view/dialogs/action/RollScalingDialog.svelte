<script lang="ts">
  ...
  function getScalingOptions() {
    return [[null, "A5E.None"], ...Object.entries(CONFIG.A5E.scalingModes)] as [
      string,
      string,
    ][];
  }
  ...
  <Section>
    <RadioGroup
      ...
      onUpdateSelection={(detail) => {
        updateDocumentDataFromField(item, `system.${propertyKey}.mode`, detail);
      }}
    />
  </Section>
    ...
    <Section heading="Increment Configuration" hint={getHint()}>
      {#if scalingMode !== "cantrip"}
        <div class="a5e-scaling-dialog__config">
          ...
          >
            Per Every {scalingMode === "spellLevel" ? "Level" : "Use"}
          </label>
        </div>
      {/if}
      ...
      {#if scalingType === "roll"}
        ...
        <div class="a5e-scaling-dialog__config">
          ...
          >
            ...
            <i
              ...
              data-tooltip="Increment by 1 to go up one size"
        ...
        <div class="a5e-scaling-dialog__config">
          <select
            ...
            name="system.{propertyKey}.config.cap"
            onchange={({ currentTarget }) => {
              updateDocumentDataFromField(
                item,
                currentTarget.name,
          ...
          >
            {#each Object.entries(CONFIG.A5E.DICE_SIDES_MAP) as [val, denom]}
              {#if val === "0"}
                <!--  -->
              {:else}
                <option value={val} selected={val == scaling.config.cap}>
                  d{denom}
                </option>
              {/if}
            {/each}
          ...
          <label
            ...
            class="a5e-scaling-dialog__label"
          >
            Dice Size Scaling Cap
            <i
              class="fa-solid fa-info-circle"
              data-tooltip="Max cap on dice size increase"
              data-tooltip-direction="UP"
            ></i>
          </label>
        </div>
      ...
      <!-- Default Types -->
      <div class="a5e-scaling-dialog__config a5e-scaling-dialog__config--long">
        <label
