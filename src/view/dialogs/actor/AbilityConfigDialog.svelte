...
<article>
  ...
  {#if !hideExpertiseDice}
    <Section heading="Ability Check Configuration">
      <ExpertiseDiePicker
        selected={actor.reactive._source.system.abilities[abilityKey]?.check
          ?.expertiseDice}
        type={actor.type}
  ...
  >
    ...
    {#if !hideExpertiseDice}
      <ExpertiseDiePicker
        selected={actor.reactive._source.system.abilities[abilityKey]?.save
          ?.expertiseDice}
        type={actor.type}
        onUpdateSelection={(value) =>
          updateDocumentDataFromField(
    ...
    {#if abilityKey === "con"}
      ...
      >
        <input
          ...
          type="text"
          value={actor.reactive.system.attributes.concentration.bonus || "0"}
          onchange={({ currentTarget }) =>
            updateDocumentDataFromField(
              actor,
              "system.attributes.concentration.bonus",
              currentTarget.value,
            )}
        />
      </FieldWrapper>
