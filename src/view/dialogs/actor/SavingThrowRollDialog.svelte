<script lang="ts">
	...
	function getInitialExpertiseDieSelection() {
		if (hideExpertiseDice) return { expertiseDie: 0, expertiseDieSource: '' };
	...
	function getSaveSrc(saveType: string, abilityKey: string) {
		...
		if (saveType === 'concentration') return actor.reactive.system.abilities.con.save;
	...
	function getSubmitButtonText(saveType: string, abilityKey: string) {
		...
		if (abilityKey === 'con' && saveType === 'concentration') {
			if (options.dc) {
				return localize('A5E.rollLabels.rollConcentrationCheckWithDC', { dc: options.dc });
			}
	...
	let visibilityMode = $state(options.visibilityMode ?? game.settings.get('core', 'messageMode'));
	...
	let rollModeData = $derived(RollOverrideManager.resolveRollMode(saveSrc, initialRollMode));
