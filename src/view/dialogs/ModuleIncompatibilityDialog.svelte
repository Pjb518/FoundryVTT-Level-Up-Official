<script lang="ts">
    import type { ModuleIncompatibilityDialog } from "./initializers/ModuleIncompatibilityDialog.svelte.ts";

    type IncompatibilityPriority = "low" | "medium" | "high";

    type Props = {
        dialog: ModuleIncompatibilityDialog;
        activeIncompatibleModules: [string, { reason: string; priority: IncompatibilityPriority }][];
    };

    let { activeIncompatibleModules }: Props = $props();
    ...
    function getPriorityColor(priorityLevel: IncompatibilityPriority): string {
    ...
    function getPriorityTooltipLabel(priorityLevel: IncompatibilityPriority): string {
        switch (priorityLevel) {
            case "low":
    ...
    let modules = $derived(
        activeIncompatibleModules.map(([module, { reason, priority }]) => [
