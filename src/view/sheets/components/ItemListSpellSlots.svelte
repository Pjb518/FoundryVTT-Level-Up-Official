<script lang="ts">
    ...
    function updateSpellSlotMax(value: number) {
        const key =
    ...
    function updateSpellSlots(level: number, slot: number) {
...
{#if level && level !== 0}
    <div
        class="a5e-spell-slot-wrapper"
        class:a5e-spell-slot-wrapper--locked={sheetIsLocked}
    >
        {#if sheetIsLocked()}
            {#each Array.from({ length: maxSpellSlots }, (_, i) => i + 1) as slot}
                ...
                <button
                    ...
                    aria-label="Update Spell Slot"
                    onclick={() => updateSpellSlots(level, slot)}
                >
                    <i class="a5e-spell-slot__icon icon fa-solid fa-star"></i>
                </button>
            {/each}
        {:else}
            <input
