...
export default class ActorGrantsManger extends Map<string, ActorGrant> {
  ...
  constructor(actor: BaseActorA5e) {
    ...
    const grantsData: Record<string, ActorGrant> =
      this.actor.system.grants ?? {};
  ...
  async createInitialGrants(
      ...
      if (levelType === "character") {
        ...
        if (item.type === "class") {
          ...
          while (true) {
            ...
            parentGrant = allGrants.find(
              (g) => g._id === parentGrant?.grantedBy?.id,
            );
    ...
    if (item.type === "class") cls = item;
      ...
      cls = this.actor.classes[item.system.class];
  ...
  async createLeveledGrants(
    ...
    const clsLevel =
      (this.actor.levels.classes?.[cls?.slug || ""] ?? 1) + difference;
    ...
    for await (const item of items) {
      ...
      allGrants.forEach((grant: Grant) => {
        ...
        const parentGrant = [...this.values()].find(
          (g) => g.grantId === grant.grantedBy?.id,
        );
        ...
        if (levelType === "character") {
          ...
          if (item.type === "class") {
            ...
            while (true) {
              ...
              if (!classParentGrant || classParentGrant.levelType === "class")
                break;
            }
        ...
        if (
          applicableGrants.find(
            (g) => this.#getFullId(g) === this.#getFullId(grant),
          )
        )
          return;

        const hasGrantedGrant = applicableGrants.find(
          (g) => g._id === grant.grantedBy?.id,
        );
  ...
  async #getSubGrants(grant: Grant, characterLevel: number): Promise<Grant[]> {
    ...
    try {
      ...
      ui.notifications?.error(
        `Grant ${grant.label} has an invalid document reference.`,
      );
      throw new Error(e);
    }
  ...
  async #applyGrants(
    ...
    const requiresConfig = [...allGrants].some((grant) =>
      grant.requiresConfig(),
    );
    const isClass = options.cls && options.item.type === "class";
    const hasSpellCasting =
      options.item?.system?.spellcasting?.ability?.options?.length;

    const requiresDialog =
      requiresConfig || !!optionalGrants.length || isClass || hasSpellCasting;
    ...
    if (!requiresDialog) {
      ...
      const { updateData, documentData } = prepareGrantsApplyData(
        this.actor,
        grants,
        new Map(),
      );
      ...
      if (!dialogData?.success) {
        if (options?.item && options.useUpdateSource && !options.cls)
          options.item.delete();
        return false;
      }
    }
      ...
      for await (const [grantId, docData] of dialogData.documentData) {
        ...
        try {
          if (docData[0]?.type === "object") {
            ...
            const existing = this.actor.items.filter((i: any) =>
              preCreateIds.includes(i.id),
            );
            ...
            updateData[`system.grants.${grantId}.documentIds`] = [
              ...ids,
              ...existingIds,
            ];
          }
    ...
    if (options.cls && options.item?.type === "class") {
      ...
      if (spellCastingAbility !== "none" && options.clsLevel === 1) {
        ...
        else if (resourceType === "inventions")
          spellBook.showSpellInventions = true;
        else if (resourceType === "artifactCharges")
          spellBook.showArtifactCharges = true;
  ...
  async removeGrantsByClassLevel(
    classLevel: number,
    slug: string,
  ): Promise<boolean> {
    const updates: Record<string, any> = {};
    ...
    for (const [, grant] of this) {
      ...
      if (originItem.type === "feature" && originItem.system.classes !== slug)
        continue;
  ...
  #getRemoveUpdates(grant: ActorGrant): Record<string, any> {
    ...
    if (
      grant instanceof actorGrants.feature ||
      grant instanceof actorGrants.item
    ) {
      let ids: string[];
      ...
      const deleteIds = this.actor.items.reduce(
        (acc: string[], i: typeof Item) => {
    ...
    if (grant instanceof actorGrants.skillSpecialty) {
      ...
      updates[`system.skills.${skill}.specialties`] = [
        ...existing.difference(removals),
      ];
    }
