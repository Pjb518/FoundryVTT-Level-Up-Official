...
class ActionsManager extends Map<string, Action> {
  ...
  get hasConsumer(): boolean {
    return [...this.values()].some(
      (action) => !!Object.values(action.consumers)?.length,
    );
  }
  ...
  get hasPrompt(): boolean {
    return [...this.values()].some(
      (action) => !!Object.values(action.prompts)?.length,
    );
  }

  get hasRange(): boolean {
    return [...this.values()].some(
      (action) => !!Object.values(action.ranges)?.length,
    );
  }

  get hasRoll(): boolean {
    return [...this.values()].some(
      (action) => !!Object.values(action.rolls)?.length,
    );
  }
  ...
  configure(actionId: string): void {
    ...
    let sheet = this.#item.dialogs.actions[actionId];
    ...
    if (!sheet) {
      ...
      this.#item.dialogs.actions[actionId] = new ActionSheet(
        this.#item,
        actionId,
        this.#item.actor,
      );

      sheet = this.#item.dialogs.actions[actionId];
    }
  ...
  async duplicate(id: string) {
    ...
    const duplicatedEffects = await this.#item.createEmbeddedDocuments(
      "ActiveEffect",
      effects,
    );
  ...
  static async addRoll(
    ...
    if (type === "attack") {
      const attackRolls = Object.values(action[1]?.rolls ?? {}).filter(
        (a) => a.type === "attack",
      );
    ...
    const updateData = {
      [`system.actions.${action[0]}.rolls`]: {
        ...action[1].rolls,
        [foundry.utils.randomID()]: foundry.utils.mergeObject(
          defaultData,
          data,
        ),
      },
  ...
  static async addConsumer(
    ...
    if (type === "spell") {
      ...
      defaultData.spellLevel = item.isType("spell")
        ? (item.system.level ?? 1)
        : 1;
