...
export default class RestManager {
  ...
  async restoreResources() {
    const { consumeSupply, haven, recoverStrifeAndFatigue, restType } =
      this.#restData;
  ...
  #consumeSupply() {
    this.#updates.actor["system.supply"] = Math.max(
      this.#actor.system.supply - 1,
      0,
    );
  }
  ...
  #restoreHitDice() {
    ...
    if (type === "actor")
      this.#updates.actor = foundry.utils.mergeObject(
        this.#updates.actor,
        updates,
      );
    if (type === "embedded")
    ...
    items.forEach((item) => {
      ...
      if (item.actions) {
        item.actions.entries().forEach(([id, action]) => {
          ...
          if (!this.restTypes.includes(actionUses?.per) || !actionUses?.max)
            return;
  ...
  #restoreSpellResources() {
    ...
    const restoreSpellPointsOnShortRest =
      flags?.restoreSpellPointsOnShortRest ?? true;
    const restoreSpellSlotsOnShortRest =
      flags?.restoreSpellSlotsOnShortRest ?? false;
