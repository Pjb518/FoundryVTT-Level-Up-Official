...
class ResourceConsumptionManager {
  ...
  #consumeActionUses({ quantity = 0 } = {}) {
    ...
    if (
      !quantity ||
      (actionUses?.value !== 0 && !actionUses?.value) ||
      !this.#actor
    )
      return;
    ...
    this.#updates.item[`system.actions.${this.#actionId}.uses.value`] =
      newValue;
  }
  ...
  #consumeItemUses({ quantity = 0 } = {}) {
    ...
    if (!max) return;
    this.#updates.item["system.uses.value"] = Math.clamp(
      value - quantity,
      0,
      max,
    );
  ...
  async #consumeQuantity(consumerId: string, consumer = {}) {
    ...
    if (deleteOnZero && newQuantity === 0) {
      // Update consumer
      this.#updates.item[
        `system.actions.${this.#actionId}.consumers.${consumerId}.itemId`
      ] = "";
      item.delete();
      return;
    }
  ...
  #consumeResource(
    ...
    if (resource === "classResource") {
      ...
      this.#updates.actor[
        `system.resources.classResources.${classIdentifier}`
  ...
  #consumeSpellResource(
    consumptionData: ResourceConsumptionManager.ConsumptionData["spell"],
  ) {
    ...
    if (consume === "spellSlot") {
      ...
      this.#updates.actor[`system.spellResources.slots.${level}.current`] =
        Math.max(value - 1, 0);
      ...
      if (this.#actor.system?.classes?.startingClass === "psyknight") {
        ...
        this.#updates.actor["system.attributes.exertion.current"] = Math.max(
          value - points,
          0,
        );
      } else {
      ...
      this.#updates.actor["system.spellResources.artifactCharges.current"] =
        Math.max(value - charges, 0);
    }
  }
  ...
  static prepareSpellData(
    ...
    if (foundry.utils.isEmpty(consumer)) {
      ...
      if (mode === "chargesOnly") spellData.consume = "artifactCharge";
        ...
        else if (availableSpellSlots.length > 0)
  ...
  static prepareUsesData(
    ...
    const itemMaxUses = getDeterministicBonus(
      itemUses.max,
      actor.getRollData(item),
    );
    const actionMaxUses = getDeterministicBonus(
      actionUses.max ?? 0,
      actor.getRollData(item),
    );
