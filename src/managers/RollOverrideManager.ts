...
export default class RollOverrideManager {
  ...
  initialize() {
    ...
    Object.entries(this.actor.system.abilities).forEach(
      ([ablKey, ability]: [string, any]) => {
        ["check", "save"].forEach((type) => {
          if (ability[type]?.expertiseDice) {
            ...
            this.overrides.get(`system.abilities.${ablKey}.${type}`)?.push({
              ...
              mode: CONFIG.A5E.ACTIVE_EFFECT_TYPES.ADD,
            });
          }
        });
      },
    ...
    this.actor.grants
      ...
      .forEach((grant: ExpertiseDiceGrant) => {
        const {
          expertiseType: type,
          expertiseCount,
          keys,
        } = grant.expertiseDiceData ?? {};
    ...
    this.actor.grants
      .byType("rollOverride")
      .forEach((grant: RollOverrideGrant) => {
        const {
          rollOverrideType: type,
          rollMode,
          keys,
        } = grant.rollOverrideData ?? {};
        const { name } = (fromUuidSync(grant.itemUuid) as FeatureItemA5e) ?? {};
        ...
        if (type === "abilityCheck") {
          keys.forEach((key: string) =>
            mapKey.push(`system.abilities.${key}.check`),
          );
        } else if (type === "abilitySave") {
          keys.forEach((key: string) =>
            mapKey.push(`system.abilities.${key}.save`),
          );
        } else if (type === "skill") {
    ...
    this.actor.items.forEach((item) => {
      ...
      if (
        ...
        this.overrides
          .get("system.skills.ste")
          ?.push({ value: -1, overrideType: "rollMode", source: item.name });
      }
    });

    // Register all overrides from effects
    const changes = this.actor.effects.reduce(
      (acc: Change[], effect: ActiveEffect) => {
        ...
        effect.changes.forEach((change) => {
          if (
            !change.key.startsWith("flags.a5e.effects.rollMode") &&
            !change.key.startsWith("flags.a5e.effects.expertiseDice")
          )
            return;
    ...
    changes.forEach((change: Change) => {
      const { key, value, name: source, type: overrideType, mode } = change;

      if (["concentration", "deathSave", "initiative"].includes(key)) {
        this.overrides.get(key)?.push({
          value,
          overrideType,
          source,
          mode,
  ...
  #prepareOverrides(
    ...
    const overrides =
      this.overrides.get(key)?.filter((o) => o.overrideType === overrideType) ??
      [];
    ...
    if (key.includes("skills")) {
      ...
      const ability =
        options.ability ||
        this.actor.system.skills[key.split(".").pop() as string]?.ability ||
        "";
      if (ability) {
        this.overrides
          .get(`system.abilities.${ability}.check`)
          ?.forEach((o) => {
            if (o.overrideType !== overrideType) return;
            overrides.push(o);
          });
    ...
    if (key.includes("attack")) {
      ...
      try {
        const flankSetting =
          game.settings.get("a5e-flanking", "flankingDND5EMode") ?? false;
  ...
  getRollOverride(
    ...
    const hasAdvantage = overrides.some(
      (o) => o.value === CONFIG.A5E.ROLL_MODE.ADVANTAGE,
    );
    const hasDisadvantage = overrides.some(
      (o) => o.value === CONFIG.A5E.ROLL_MODE.DISADVANTAGE,
    );

    if (hasAdvantage && hasDisadvantage) return CONFIG.A5E.ROLL_MODE.NORMAL;
    if (!hasAdvantage && !hasDisadvantage) return baseRollMode;
    if (hasAdvantage)
      return this.#determineRollMode(
        baseRollMode,
  ...
  getRollOverridesSource(
    ...
    else if (baseRollMode === CONFIG.A5E.ROLL_MODE.DISADVANTAGE)
      base = "Disadvantage";
    else base = "Normal";

    const { adv, dis } = overrides.reduce(
      (acc: { adv: string[]; dis: string[] }, o) => {
        if (o.value === CONFIG.A5E.ROLL_MODE.ADVANTAGE) acc.adv.push(o.source);
        else if (o.value === CONFIG.A5E.ROLL_MODE.DISADVANTAGE)
          acc.dis.push(o.source);
    ...
    if (adv.length)
      advString = `<p> <strong>Advantage:</strong> ${adv.join(", ")}</p>`;
    if (dis.length)
      disString = `<p> <strong>Disadvantage:</strong> ${dis.join(", ")}</p>`;
  ...
  getExpertiseDice(
    ...
    let total = baseDie;
    overrides.forEach((o) => {
      if (o.mode === CONFIG.A5E.ACTIVE_EFFECT_TYPES.OVERRIDE) total = o.value;
      else total += o.value;
    });
  ...
  getExpertiseDiceSource(
    ...
    const base = baseDie
      ? `d${CONFIG.A5E.expertiseDiceSidesMap[baseDie]}`
      : "None";
    const sources = overrides.map(
      (o) => `${o.source} (d${CONFIG.A5E.expertiseDiceSidesMap[o.value]})`,
    );
