...
class A5eEnricherManager {
    ...
    CONFIG.TextEditor.enrichers.push(
      {
        // [[/check args=d|"d"|'d']]
        pattern:
          /\[\[\/(?<enricherType>check|save|condition)(?<argString> [^\]]+)?]]/gi,
  ...
  async parseEnricherInput(
    ...
    const { enricherType, argString } = match.groups as {
      enricherType: string;
      argString: string;
    };
  ...
  async parseChooseInput(
    ...
    const { flavor, argString } = match.groups as {
      flavor: string;
      argString: string;
    };
  ...
  async #enrichCheck(
    ...
    if (!skill && !ability)
      return this.createInvalidTag("No ability or skill provided.");
  ...
  async #enrichSave(
    ...
    if (!type && !ability)
      return this.createInvalidTag("No type or ability provided.");

    // Is either concentration or death save
    if (type) {
      if (!saveTypes.includes(type))
        return this.createInvalidTag(`Invalid save type ${type}.`);
  ...
  async #createRollButton(
    ...
    // @ts-expect-error
    if (args.dc)
      dc = (await new Roll(args.dc, options.rollData).evaluate()).total;
    const showDC = (game.user?.isGM || options?.secrets) && dc;

    if (showDC) {
      span.innerHTML = `<i class="icon fa-solid fa-dice-d20"></i><span class="a5e-enricher__dc">DC ${dc}</span>${label}`;
    } else {
      span.innerHTML = `<i class="icon fa-solid fa-dice-d20"></i>${label}`;
    }
        ...
        for (const selectedToken of selectedTokens) {
          ...
          if (!actor) {
            ui.notifications?.error(
              `No actor found on given token "${selectedToken.name}"`,
            );
            continue;
          }
  ...
  async #enrichChoose(
    ...
    for (const arg of args) {
      ...
      const weight = arg.groups?.weight
        ? Number.parseInt(arg.groups.weight, 10)
        : 1;
    ...
    span.innerHTML = `<i class="icon fas fa-th-list"></i>${label}`;
    return span;
  }
