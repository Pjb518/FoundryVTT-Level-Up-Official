...
export default class ModifierManager {
  ...
  #getInitiativeRollModifiers() {
    if (
      game.settings.storage.get("world").getItem("a5e.simpleInitiative") ??
      false
    ) {
  ...
  #getAbilityCheckProficiencyBonus() {
    ...
    if (
      game.settings.storage
        .get("world")
        .getItem("a5e.5eStyleJackOfAllTrades") ??
      false
    ) {
  ...
  #getGlobalAttackBonus() {
    ...
    if (selectedAttackBonuses) {
      value = BonusesManager.getSelectedBonusesFormula(
        "attacks",
  ...
  #getSkillCheckModifier() {
    ...
    return {
      label: labelKey
        ? localize(labelKey, { skill: CONFIG.A5E.skills[skillKey] })
        : "",
