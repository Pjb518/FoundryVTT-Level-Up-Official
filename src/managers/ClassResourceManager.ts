...
export default class ClassResourceManager extends Map<string, ClassResource> {
  ...
  constructor(item: ClassItemA5e | ArchetypeItemA5e) {
    ...
    const classResourceData =
      (this.item.system.resources as ClassResource[]) ?? [];

    classResourceData.forEach((data: ClassResource) => {
      const classResource = data;
      const slug =
        classResource.slug || classResource.name.slugify({ strict: true });
  ...
  prepareResources() {
    ...
    [...this.entries()].forEach(([slug, resource]) => {
      ...
      try {
        const doc = this.item.isEmbedded
          ? (this.item.parent ?? this.item)
          : this.item;
  ...
  async remove(slug: string) {
    const filteredArray = this.item.system.resources.filter(
      (resource) =>
        resource.slug !== slug ||
        resource.name?.slugify({ strict: true }) !== slug,
    );
