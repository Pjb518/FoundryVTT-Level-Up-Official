...
export default class BonusesManager {
	...
	getDefaultSelections(key: string, data: SelectionData = {}): string[] {
		...
		return parts.reduce((acc: string[], [id, value]: [string, any]) => {
	...
	prepareHitPointsBonuses(): [string, HitPointsBonus][] {
		...
		return Object.entries(bonuses).reduce((acc: [string, HitPointsBonus][], [key, bonus]) => {
	...
	prepareSensesBonuses(type: string): [string, SensesBonus][] {
		...
		const parts = Object.entries(bonuses).filter(([, { context, formula, unit }]) => {
	...
	prepareSkillBonuses(
		...
		const abilityParts: [string, AbilityBonus][] = this.prepareAbilityBonuses(
