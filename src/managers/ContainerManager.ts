...
export default class ContainerManager extends Map<string, SubObjectField> {
  ...
  constructor(item: InstanceType<typeof ObjectItemA5e>) {
    ...
    const containerData: [string, SubObjectField][] = Object.entries(
      this.#item.system.items ?? {},
    );
  ...
  get items() {
    ...
    if (parent.pack) {
      const p: Promise<(unknown | null)[]> = Promise.all(
        docUuids.map((uuid) => fromUuid(uuid)),
      );
  ...
  get allItems(): any[] | Promise<any[]> {
    ...
    items.forEach((i) => {
      ...
      if (i?.system?.objectType === "container") {
        ...
        if (subItems instanceof Promise)
          subItems.then((si) => all.push(...(si ?? [])));
  ...
  get count(): number | Promise<number> {
    ...
    if (contents instanceof Promise) {
      return contents.then((items) =>
        items.reduce((acc, i) => acc + i.system.quantity, 0),
      );
    }
  ...
  get weight(): number | Promise<number> {
    const hasWeightlessContents =
      this.#item.system?.capacity?.weightlessContents ?? false;
    ...
    return contents.reduce(
      (acc, i) => acc + i.system.quantity * i.system.weight,
      0,
    );
  }
  ...
  static async createContainerOnActor(actor: any, item: any): Promise<any> {
    ...
    const containerData: Array<any> = foundry.utils.duplicate(
