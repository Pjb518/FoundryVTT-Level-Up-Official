...
export default function getSummaryData(
  ...
  if (foundry.utils.isEmpty(action)) {
    ...
    else if (item.isType('object')) mergeObject(summaryData, getObjectSummaryData(item as ObjectItemA5e, options));
    else if (item.isType('spell')) mergeObject(summaryData, getSpellSummaryData(item as SpellItemA5e, options));
  }
