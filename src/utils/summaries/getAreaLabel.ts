...
export default function getAreaLabel(action: Action) {
  ...
  if (area.shape === "circle") {
    ...
    areaLabel = localize("A5E.AreaWallSpecific", {
      height: area.height,
      length: area.length,
      width: area.width,
    });
  } else if (area.shape === "wall" && area.height) {
    areaLabel = localize("A5E.AreaWallSpecificNoWidth", {
      height: area.height,
      length: area.length,
    });
  } else if (area.shape === "wall" && area.width) {
    areaLabel = localize("A5E.AreaWallSpecificNoHeight", {
      length: area.length,
      width: area.width,
    });
  } else if (area.shape === "wall") {
    areaLabel = localize("A5E.AreaWallSpecificNoWidthOrHeight", {
      length: area.length,
    });
  } else {
    return "";
