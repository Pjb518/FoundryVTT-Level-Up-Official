...
export default function getAreaLabel(action: Action) {
  ...
  if (area.shape === "circle") {
    ...
    areaLabel = localize("A5E.area.wallSpecificNoWidth", {
      height: area.height,
      length: area.length,
    });
    ...
    areaLabel = localize("A5E.area.wallSpecificNoWidthOrHeight", {
      length: area.length,
    });
  } else {
    return "";
