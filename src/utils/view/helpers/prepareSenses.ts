export function prepareSenses(data: any): string[] {
  const senses = Object.entries(data.system.attributes.senses).filter(
    ([sense, senseData]) =>
  ...
  return senses.map(([sense, senseData]) => {
    ...
    const unitLabel = game.i18n.localize(
      CONFIG.A5E.distanceAbbreviations[senseData.unit],
    );
    ...
    if (senseData.unit === "unlimited") {
      const unlimitedLabel = game.i18n.localize(
        CONFIG.A5E.visionUnits.unlimited,
      );
