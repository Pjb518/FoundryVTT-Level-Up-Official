...
export function prepareWeaponProficiencies(data: any): string[] {
  const weaponProficienciesByGroup = data.system.proficiencies.weapons.reduce(
    (acc, curr) => {
      if (Object.keys(CONFIG.A5E.weaponsPlural.martial).includes(curr)) {
        ...
        Object.keys(CONFIG.A5E.weaponsPlural.miscellaneous).includes(curr)
      ) {
  ...
  ["simple", "martial", "rare", "miscellaneous"].forEach((weaponType) => {
    ...
    if (arraysAreEqual(weaponKeys, weaponProficienciesByGroup[weaponType])) {
      ...
      weaponProficienciesByGroup[weaponType] = weaponProficienciesByGroup[
        weaponType
      ].map((weapon) =>
  ...
  const weaponProficiencies = Object.values(
    weaponProficienciesByGroup,
  ).flat() as string[];
  weaponProficiencies.sort((a, b) =>
