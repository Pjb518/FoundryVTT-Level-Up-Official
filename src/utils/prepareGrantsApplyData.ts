...
import type CharacterActorA5E from "../documents/actor/character.ts";
...
export default function prepareApplyData(
  ...
  grants.forEach(({ id, grant }: { id: string; grant: Grant }) => {
    ...
    if (grant.grantType === "feature") {
      ...
      const uuids: string[] =
        inputData?.uuids ?? grant.features.base.map(({ uuid }) => uuid) ?? [];
    ...
    Object.entries(grantUpdates ?? {}).forEach(([key, value]) => {
      ...
      const originalValue =
        (foundry.utils.getProperty(updateData, key) as string[]) ?? [];
