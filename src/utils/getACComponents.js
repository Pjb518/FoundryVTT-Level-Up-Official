...
export default function getACComponents(actor) {
  ...
  const effectChanges = actor.effects.reduce((acc, effect) => {
    ...
    const changes = effect.changes.reduce((changesAcc, change) => {
      if (change.key !== "system.attributes.ac.changes.bonuses.value")
        return changesAcc;
  ...
  if (baseChanges.override.formula) {
    const formulaTerms = new Roll(
      baseChanges.override.formula,
      actor.getRollData(),
    ).evaluateSync().terms;

    const formula = simplifyOperatorTerms(formulaTerms ?? []).reduce(
      (acc, term) => {
