...
import { simplifyOperatorTerms } from '../dice/simplifyOperatorTerms.ts';
...
export function getACComponents(actor: Actor) {
	...
	const effectChanges = actor.effects.reduce((acc, effect) => {
		...
		const changes = effect.changes.reduce((changesAcc, change) => {
	 ...
	 */
	let changes;
	if (effectChanges.find(({ mode }) => mode === CONFIG.A5E.ACTIVE_EFFECT_TYPES.OVERRIDE)) {
		changes = effectChanges;
	} else {
		changes = (baseChanges.bonuses.components ?? []).concat(
			baseChanges.bonuses.value ? effectChanges : [],
		);
	}
	...
	if (baseChanges.override.formula) {
		const formulaTerms = new Roll(baseChanges.override.formula, actor.getRollData()).evaluateSync()
			.terms;
	...
	const acFormula = simplifyOperatorTerms(
		new Roll(components.join(' '), actor.getRollData()).evaluateSync().terms ?? [],
	);
