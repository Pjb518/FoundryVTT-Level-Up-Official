...
export default async function openCompendium(
	...
	if (!hideDialog && packOptions.length > 1) {
		const dialog = new GenericDialog('Select Compendium', ImportCompendiumSelectionDialog, {
			packOptions,
			defaultSelection: CONFIG.A5E.defaultActorImportCompendia[tab],
		});
	...
	const importer = importFunction ?? ((docs: any[]) => actor.createEmbeddedDocuments('Item', docs));
